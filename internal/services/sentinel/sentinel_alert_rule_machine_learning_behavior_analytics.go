...
func resourceSentinelAlertRuleMLBehaviorAnalyticsCreateUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if d.IsNewResource() {
		...
		model := resp.Model
		if model == nil {
			return fmt.Errorf("reading existing Sentinel Alert Rule MLBehaviorAnalytics %q: model is nil", id)
		}
	...
	if !d.IsNewResource() {
		...
		if err := assertAlertRuleKind(model, alertrules.AlertRuleKindMLBehaviorAnalytics); err != nil {
...
func resourceSentinelAlertRuleMLBehaviorAnalyticsRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	rule := (*model).(alertrules.MLBehaviorAnalyticsAlertRule)
