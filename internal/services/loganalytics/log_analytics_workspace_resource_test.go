...
func TestAccLogAnalyticsWorkspace_ToggleDataExport(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_log_analytics_workspace", "test")
	r := LogAnalyticsWorkspaceResource{}

	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.withDataExport(data, true),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
		{
			Config: r.withDataExport(data, false),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func (LogAnalyticsWorkspaceResource) withDataExport(data acceptance.TestData, enable bool) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
