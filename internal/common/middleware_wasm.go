...
func responseLoggerMiddleware(providerName string) client.ResponseMiddleware {
	return func(request *http.Request, response *http.Response) (*http.Response, error) {
		...
		if dump, err2 := httputil.DumpResponse(response, true); err2 == nil {
			// print to stdout/err will cause many times memory copy(go->js(writeSync)->go(writeSync)->js(postMessage)->go(client)
			// and might block on the `postMessage` func, so we print it directly to js console.
			js.Global().Get("console").Call("log", fmt.Sprintf("[DEBUG] %s Response for %s: \n%s\n", providerName, request.URL, dump))
		} else {
			// fallback to basic message
			js.Global().Get("console").Call("log", fmt.Sprintf("[DEBUG] %s Response: %s for %s\n", providerName, response.Status, request.URL))
		}
		return response, nil
	}
}
