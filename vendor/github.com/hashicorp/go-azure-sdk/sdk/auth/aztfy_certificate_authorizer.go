...
type AztfyCertificateAuthorizerOptions struct {
	...
	// KnownToken is the token obtained previously, to reduce the initial token request
	KnownToken *oauth2.Token
}

// NewClientCertificateAuthorizer returns an authorizer which uses client certificate authentication.
func NewAztfyCertificateAuthorizer(ctx context.Context, options AztfyCertificateAuthorizerOptions) (Authorizer, error) {
	...
	return NewCachedAuthorizerWithKnownToken(&ClientAssertionAuthorizer{
		conf: &conf,
	}, options.KnownToken)
}
